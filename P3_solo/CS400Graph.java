...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            // new distance is the existing distance plus new weight
            this.distance = copyPath.distance + extendBy.weight;

            // copy over the data sequence from copyPath
            LinkedList<T> list = new LinkedList<T>();
            for (T v : copyPath.dataSequence) {
                list.add(v);
            }

            // add the new node and set this dataSequence to be our new one
            list.add(extendBy.target.data);
            this.dataSequence = (List<T>) list;
    ...
    protected Path dijkstrasShortestPath(T start, T end) {        
        ...
        if (!this.vertices.containsKey(start))
            throw new NoSuchElementException("Start vertex could not be found");
        if (!this.vertices.containsKey(end))
            throw new NoSuchElementException("End vertex could not be found");

        // find the vertices corresponding to the beginning and ending data
        Vertex startVertex = this.vertices.get(start);
        Vertex endVertex = this.vertices.get(end);

        // make a new Hashtable to associate vertices with their distances
        Hashtable<CS400Graph<T>.Vertex, Boolean> visitedNodes = new Hashtable<CS400Graph<T>.Vertex, Boolean>();

        // create our new PriorityQueue and instantiate it with the 
        // starting node
        PriorityQueue<CS400Graph<T>.Path> pq = new PriorityQueue<CS400Graph<T>.Path>();
        pq.add(new Path(startVertex));

        while (pq.size() > 0) {
            // get the top path from the priority queue
            CS400Graph<T>.Path cur = pq.poll();

            // if we previously found a more costly route to this node but have since
            // found a cheaper one, then skip past this
            if (visitedNodes.containsKey(cur.end))
                continue;

            // if we found the node we're looking for, and since we know that with
            // dijkstra's we have the shortest possible path when we discover a node,
            // then we can confidently return our current path
            if (cur.end.equals(endVertex))
                return cur;

            // expand its edges and add all of those paths to the priority queue
            for (CS400Graph<T>.Edge e : cur.end.edgesLeaving) {
                // only add the path to the PQ if we haven't discovered it before,
                // we know for sure we've found the cheapest path to the node
                // when computing with dijkstra's, so if we've visited it before
                // then we can't possibly be finding a new, cheaper path
                if (!visitedNodes.containsKey(e.target))
                    pq.add(new Path(cur, e));
            }

            // we definitely haven't seen this node before, and we are confident
            // that the path we have now is the shortest one to this node, so 
            // update its distance in our table
            visitedNodes.put(cur.end, true);
        }

        // if a path is not possible, then we throw a NoSuchElementException
        throw new NoSuchElementException("No path from start node to end node can be found");
    }
